<template>
    <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg mt-4">
        <div class="px-4 py-5 sm:px-6 flex justify-between">
            <div class="">
                <h2 id="applicant-information-title" class="text-3xl leading-6 font-bold text-gray-600 dark:text-gray-100">Create Crypto Education</h2>
        ...
        <div class="px-4 py-5 sm:px-6">
            <div class="shadow overflow-hidden sm:rounded-lg">
                <form class="space-y-8" @submit.prevent="newEducation">
                    <div class="space-y-8">
                    <div class="pt-8">
                        <div>
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-200">Create New Course</h3>
                        </div>
                        <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            <div class="sm:col-span-4">
                                <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-200"> Title </label>
                                <div class="mt-1">
                                <input id="title" name="title" type="text" autocomplete="off" class="shadow-sm focus:ring-red-500 focus:border-red-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-200" v-model="forms.title" required>
                                <p v-if="errors.title" class="mt-2 text-sm text-red-600 dark:text-red-300">{{ errors.title[0] }}</p>
                                </div>
                            </div>
                            <div class="sm:col-span-2">
                                <label for="category_id" class="block text-sm font-medium text-gray-700 dark:text-gray-200"> Category </label>
                                <div class="mt-1">
                                <select id="category_id" name="category_id" v-model="forms.category_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:bg-gray-800 dark:text-gray-200 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm rounded-md" placeholder="Choose category" required>
                                    <option v-for="category in categories" :key="category.id" :value="category.id">{{ category.name }}</option>
                                </select>
                                <p v-if="errors.category_id" class="mt-2 text-sm text-red-600 dark:text-red-300">{{ errors.category_id[0] }}</p>
                                </div>
                            </div>
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <div class="mt-1">
                                    ...
                                    <p v-if="errors.content" class="mt-2 text-sm text-red-600 dark:text-red-300">{{ errors.content[0] }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const { errors, categories, storeEducation, fetchCategories } = useAdminEducation()
        ...
        const newEducation = async () => {
            ...
            photo.append('category_id', forms.category_id)

            await storeEducation(photo).then(() => {
