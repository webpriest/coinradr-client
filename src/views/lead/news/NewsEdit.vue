<template>
    <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg mt-4">
        <div class="px-4 py-5 sm:px-6 flex justify-between">
            <div class="">
                <h2 id="applicant-information-title" class="text-2xl leading-6 font-bold text-gray-600 dark:text-gray-100">{{ post.caption }}</h2>
        ...
        <div class="px-4 py-5 sm:px-6">
            <div class="shadow overflow-hidden sm:rounded-lg">
                <form class="space-y-8" @submit.prevent="editPost">
                    <div class="space-y-8">
                    <div class="pt-8">
                        <div class="flex justify-between">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-200">Edit News Article</h3>

                            <div class="flex">
                                <span class="text-gray-800 dark:text-gray-200 font-medium mr-2">Published</span>
                                <Switch v-model="post.published" :class="[post.published ? 'bg-red-600 dark:bg-red-300' : 'bg-gray-200 dark:bg-gray-500', 'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none']">
                                    <span class="sr-only">Use setting</span>
                                    <span :class="[post.published ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none relative inline-block h-5 w-5 rounded-full bg-white dark:bg-gray-900 shadow transform ring-0 transition ease-in-out duration-200']">
                                    <span :class="[post.published ? 'opacity-0 ease-out duration-100' : 'opacity-100 ease-in duration-200', 'absolute inset-0 h-full w-full flex items-center justify-center transition-opacity']" aria-hidden="true">
                                        <svg class="h-3 w-3 text-gray-400 dark:text-gray-200" fill="none" viewBox="0 0 12 12">
                                        <path d="M4 8l2-2m0 0l2-2M6 6L4 4m2 2l2 2" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </span>
                                    <span :class="[post.published ? 'opacity-100 ease-in duration-200' : 'opacity-0 ease-out duration-100', 'absolute inset-0 h-full w-full flex items-center justify-center transition-opacity']" aria-hidden="true">
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            <div class="sm:col-span-4">
                                <label for="caption" class="block text-sm font-medium text-gray-700 dark:text-gray-200"> News Caption </label>
                                <div class="mt-1">
                                <input id="caption" type="text" autocomplete="off" class="shadow-sm focus:ring-red-500 focus:border-red-500 block w-full sm:text-sm border-gray-500 rounded-md dark:bg-gray-800 dark:text-gray-200" v-model="post.caption">
                                </div>
                            </div>
                            <div class="sm:col-span-2">
                                <label for="author" class="block text-sm font-medium text-gray-700 dark:text-gray-200"> Author </label>
                                <div class="mt-1">
                                <input id="author" type="text" autocomplete="off" class="shadow-sm focus:ring-red-500 focus:border-red-500 block w-full sm:text-sm border-gray-500 rounded-md dark:bg-gray-800 dark:text-gray-200" v-model="post.author">
                                </div>
                            </div>
    ...
    setup(props) {
        ...
        const editPost = async () => {
            if(imageFile.value.length !== 0) {
                ...
                postData.append('photo', imageFile.value)
                postData.append('caption', post.value.caption)
                postData.append('content', post.value.content)
                postData.append('author', post.value.author)
                postData.append('published', (post.value.published == true) ? 1 : 0)
                postData.append('_method', 'PATCH')

                let response = await axios.post(`/api/admin/posts/${props.slug}`, postData)

                // router.push({ name: 'news.show', params: { slug: response.data.data.slug } })
            }
            else {
                let response = await axios.patch(`/api/admin/posts/${props.slug}`, post.value)
