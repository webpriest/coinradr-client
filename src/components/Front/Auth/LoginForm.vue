              ...
              <div>
                ...
                <div class="isolate -space-y-px rounded-md shadow-sm mt-4">
                  <div class="relative border border-red-300 rounded-md rounded-b-none px-3 py-2 focus-within:z-10 focus-within:ring-1 focus-within:ring-red-600 focus-within:border-red-600">
                    ...
                    <input type="email" id="email" class="block w-full border-0 focus:outline-none border-gray-100 p-0 text-gray-900 placeholder-gray-500 focus:ring-0 sm:text-sm" placeholder="jane.doe@coinradr.com" autocomplete="off" v-model="forms.email"  />
...
export default {
  ...
  setup(props) {
    const loginFormStatus = props.loginFormStatus
      console.log(props.loginFormStatus)
    const store = useStore()
    const router = useRouter()

    const loginFormLoad = () => {
      loginFormStatus.value = props.loginFormStatus
    }
    ...
    const attemptLogin = () => {
      store.dispatch('login', forms).then(() => {
        loginFormLoad()
